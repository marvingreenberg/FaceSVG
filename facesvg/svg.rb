    ...
    class Canvas
      ...
      def add_paths(xf, face, surface)
        ...
        if face.material == FaceSVG.pocket
          ...
          cut_depth = FaceSVG::cfg().cut_depth
    ...
    class PathAttributes
      def initialize(kind, cut_depth)
        if kind == PK_EXTERIOR
          ...
          @attributes = {
            SHAPER_PATH_TYPE => kind, SHAPER_CUT_DEPTH => format(FMT, cut_depth),
            FILL => _rgb(FaceSVG::cfg().fill_interior_color), STROKE => _rgb(FaceSVG::cfg().stroke_interior_color),
          ...
          @attributes = { SHAPER_PATH_TYPE => kind,
            STROKE_WIDTH => '2', STROKE => FaceSVG::cfg().annotation_color,
            VECTOR_EFFECT => VE_NON_SCALING_STROKE }
        end
      end
      ...
      def color_range(depth)
        offs = [100.0*depth/FaceSVG::cfg().pocket_max, 100].min.to_int
        _rgb(FaceSVG::cfg().pocket_color_values.map { |x| [0, x+offs].min })
      rescue
        return 'rgb(165,165,165)'
      end
      def pocket_stroke(depth)
        return _rgb(FaceSVG::cfg().stroke_pocket_color) if FaceSVG::cfg().stroke_pocket_color
        color_range(depth)
      end
      def pocket_fill(depth)
        return _rgb(FaceSVG::cfg().fill_pocket_color) if FaceSVG::cfg().fill_pocket_color
        color_range(depth)
      end
